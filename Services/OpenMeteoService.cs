...
namespace WeatherMap.Services
{
    ...
    /// <summary>
    /// Implementação do serviço de integração com Open-Meteo API
    /// </summary>
    public class OpenMeteoService : IWeatherService
    {
            ...
            CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                _logger.LogInformation("URL do forecast: {Url}", url);
                _logger.LogInformation("BaseAddress do HttpClient: {BaseAddress}", _httpClient.BaseAddress);
            ...
            CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                var geocodingUrl = $"{_apiSettings.GeocodingBaseUrl}/search?name={Uri.EscapeDataString(cityName)}&count=10&language=pt&format=json";
                ...
                _logger.LogInformation("URL de geocoding: {Url}", geocodingUrl);

                // Criar um HttpClient específico para geocoding (não usar o configurado para forecast)
                using var geocodingClient = new HttpClient();
                var response = await geocodingClient.GetAsync(geocodingUrl, cancellationToken);

                if (!response.IsSuccessStatusCode)
                {
                    _logger.LogError("Erro na API de Geocoding: {StatusCode} - {ReasonPhrase}",
            ...
            CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                if (!response.IsSuccessStatusCode)
                {
                    _logger.LogError("Erro na API Open-Meteo Historical: {StatusCode} - {ReasonPhrase}",
                        response.StatusCode, response.ReasonPhrase);
                    return null;
                }
